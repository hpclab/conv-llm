...
if False:
    ...
    qrels19 =load_qrels('2019')
    qrels20 =load_qrels('2020')



    eval_20 = pd.read_csv('./trec/evaluation2020.csv') 
    eval_20 = eval_20[eval_20.qid.isin(qrels20.qid.unique())].rename(columns={'number':'qid','raw_utterance':'query'})
    eval_19 = pd.read_csv('./trec/evaluation2019.csv').rename(columns={'number':'qid','raw_utterance':'query'})
    eval_19[['qid','query']].to_csv('ultima_prova/rewritings/cleaned/Original_2019.tsv',sep='\t', index=False)
    eval_20[['qid','query']].to_csv('ultima_prova/rewritings/cleaned/Original_2020.tsv',sep='\t', index=False)



    cqr_rule_based_cv = extract_topics_cqr(pd.DataFrame(read_json('cqr/ConversationQueryRewriter/results/query_rewriter_output_rule_based_cv.jsonlines')))
    cqr_self_learn_cv = extract_topics_cqr(pd.DataFrame(read_json('cqr/ConversationQueryRewriter/results/query_rewriter_output_self_learn_cv.jsonlines')))
    cqr_rule_based_cv =cqr_rule_based_cv[cqr_rule_based_cv.qid.isin(qrels19.qid.unique())]
    cqr_self_learn_cv =cqr_self_learn_cv[cqr_self_learn_cv.qid.isin(qrels19.qid.unique())]

    cqr_rule_based_cv = cqr_rule_based_cv.merge(eval_19[['qid','query']].rename(columns={'query':'original'}), on='qid', how ='right',)
    cqr_self_learn_cv = cqr_self_learn_cv.merge(eval_19[['qid','query']].rename(columns={'query':'original'}), on='qid', how ='right')
    cqr_rule_based_cv['query'] = cqr_rule_based_cv['query'].fillna(cqr_rule_based_cv['original'])
    cqr_self_learn_cv['query'] = cqr_self_learn_cv['query'].fillna(cqr_self_learn_cv['original'])

    cqr_rule_based_cv[['qid','query']].to_csv('ultima_prova/rewritings/cleaned/cqr_rule_based_cv_2019.tsv', sep ='\t', index=False)
    cqr_self_learn_cv[['qid','query']].to_csv('ultima_prova/rewritings/cleaned/cqr_self_learn_cv_2019.tsv', sep ='\t', index=False)


    quretec20 = pd.read_csv('QURETEC_2020_complete.tsv',sep='\t')
    quretec20 = quretec20.rename(columns= {'conversation_id':'conv_id','turn_id':'turn',"id":'qid'})
    quretec20 = quretec20[quretec20.qid.isin(qrels20.qid.unique())]
    quretec20[['qid','query']].to_csv('./ultima_prova/rewritings/cleaned/QURETEC_2020.tsv',sep='\t',index=False)

    manual_20 = create_df_from_json(pd.read_json('trec/treccast/2020_manual_evaluation_topics_v1.0.json'))
    manual_20 = manual_20[manual_20.number.isin(qrels20.qid.unique())][['number','manual_rewritten_utterance']].rename(columns={'number':'qid','manual_rewritten_utterance':'query'})
    manual_20[['qid','query']].to_csv('ultima_prova/rewritings/cleaned/Manual_2020.tsv',sep='\t', index=False)
    manual_19 = pd.read_csv('trec/treccast/test_manual_utterance.tsv', sep ='\t', names = ['qid','query'])
    manual_19 = manual_19[manual_19.qid.isin(qrels19.qid.unique())]
    manual_19[['qid','query']].to_csv('ultima_prova/rewritings/cleaned/Manual_2019.tsv',sep='\t', index=False)
